...
export default function ProfileView() {
  const { profile, education, languages, skillGroups } = useSiteContent();
  ...
  // Deduplicate skills across groups (e.g. "Java" appears in more than one group).
  const coreSkills = Array.from(new Set(skillGroups.flatMap((g) => g.skills))).slice(0, 22);
        ...
        >
          <div className="flex flex-col items-center gap-7 text-center md:flex-row md:items-center md:text-left">
            ...
            <div className="flex-1">
              ...
              <p className="mt-2 font-mono text-sm text-neon-cyan sm:text-base">{profile.title}</p>
        ...
        <div className="mt-8 grid gap-6 lg:grid-cols-5">
          ...
          >
            <div className="glass rounded-2xl p-6">
              ...
              <p className="text-sm leading-relaxed text-fg-muted md:text-[15px]">{profile.bio}</p>
              <p className="mt-3 text-sm leading-relaxed text-fg-muted md:text-[15px]">{profile.bioShort}</p>
            </div>
